...
**当前仓库中是 Version 2.0 的代码。在本次任务中，你需要完成 Version 2.1 代码的编写。Version 2.0的要求仅供参考。**

## Version 2.0 任务要求

- 删除 Gradio 前端中的“Overlap Frames”模块；删除后端中与之相关的逻辑，且确保不要影响其他功能。
- （**Deprecated**）原来代码中Primitives中的坐标都归一化到[0, 1]，我希望归一化到[0, 1000]，有效数字是一样的，但用整数而不是浮点数来表示。Primitives的其他格式不变。请仔细检查代码，在所有相关位置（包括但不限于calibration、annotation、历史记录保存、数据集导出等）完成这一修改。
- 依次添加下面两个功能：
...
- 请完善代码：**代码中 Primitives 中的坐标仍然归一化到[0, 1]**，不要归一化到[0, 1000]，有效数字保持不变（三位小数）。Primitives的其他格式不变。请仔细检查代码，在所有相关位置（包括但不限于calibration、annotation、历史记录保存、数据集导出等）完成这一修改。你可以参照 git 中 Version 2.0 之前的最新版本的历史记录。
- 其他地方同 Version 2.0。
- 完善好代码后，更新 README.md 文档（该版本标记为 2.1）。
